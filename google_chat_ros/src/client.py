...
class GoogleChatClient(object, keyfile):
    def __init__(self):
    ...
    def authenticate(self):
        """Authenticate Google REST API by json key file.
    ...
    def send_text(self, space, text):
        parent = 'spaces/' + space
        self.res = self._chat.spaces().messages().create(parent=parent, body={'text':text}).execute() # returns same 403 error both authenticate error and not connected error
