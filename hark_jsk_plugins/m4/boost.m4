...
# BOOST_FILESYSTEM_JUDGE([VERSION], [ACTION-IF-NOT-FOUND])
# -----------------------------------------------
# Look for Boost.  If version is given, it must either be a literal of the form
# "X.Y.Z" where X, Y and Z are integers (the ".Z" part being optional) or a
# variable "$var".
...
# ACTION-IF-NOT-FOUND action if one was supplied.
# Otherwise aborts with an error message.
AC_DEFUN([BOOST_FILESYSTEM_JUDGE],
[AC_REQUIRE([AC_PROG_CXX])dnl
AC_REQUIRE([AC_PROG_GREP])dnl
echo "$as_me: this is boostapd.m4[]_BOOST_SERIAL" >&AS_MESSAGE_LOG_FD
boost_save_IFS=$IFS
boost_version_req=$1
IFS=.
set x $boost_version_req 0 0 0
...
AC_SUBST([DISTCHECK_CONFIGURE_FLAGS],
         ["$DISTCHECK_CONFIGURE_FLAGS '--with-boost=$with_boost'"])dnl
boost_save_CPPFLAGS=$CPPFLAGS
  AC_CACHE_CHECK([for Boost headers version for filesytem >= $boost_version_req_string],
    [boost_cv_inc_path],
    [boost_cv_inc_path=no
AC_LANG_PUSH([C++])dnl
m4_pattern_allow([^BOOST_VERSION$])dnl
...
AC_LANG_POP([C++])dnl
    ...
    case $boost_cv_inc_path in #(
      no)
        boost_errmsg="cannot find Boost headers version for boost_filesystem >= $boost_version_req_string"
	AC_DEFINE([BOOST_FILESYSTEM_VERSION], [2], [to use the boost_file_system ver2])
        ;;#(
      yes)
	AC_DEFINE([BOOST_FILESYSTEM_VERSION], [3], [to use the boost_file_system ver3])
	AC_DEFINE([BOOST_FILESYSTEM_VERSION_3], [1], [be defined for ifdef in order to maintain the compatibility of version 3 from 2 of the boost_file_system])
        ;;
    esac
])# BOOST_FILESYSTEM_JUDGE

# BOOST_THREAD_JUDGE([VERSION], [ACTION-IF-NOT-FOUND])
# -----------------------------------------------
# Look for Boost.  If version is given, it must either be a literal of the form
# "X.Y.Z" where X, Y and Z are integers (the ".Z" part being optional) or a
# variable "$var".
...
AC_LANG_POP([C++])dnl
    ...
    case $boost_cv_inc_path in #(
      no)
        boost_errmsg="cannot find Boost headers version for boost_thread >= $boost_version_req_string"
        AC_SUBST([BOOST_THREAD],
         ["$BOOST_THREAD -lboost_thread-mt"])dnl
        ;;#(
      yes)
        AC_SUBST([BOOST_THREAD],
         ["$BOOST_THREAD -lboost_thread"])dnl
        ;;
    esac
])# BOOST_THREAD_JUDGE
