...
class GoogleCloudTextToSpeechPlugin(SoundPlayPlugin):
    def __init__(self):
        super(GoogleCloudTextToSpeechPlugin, self).__init__()
    ...
    def sound_play_say_plugin(self, text, voice):
        ...
        name, language_code = self.determine_voice_name(voice)

        if cache_enabled:
            cache_dir = self.get_cache_dir()
            md5 = self.checksum_md5(txtfilename)
            ...
            if os.path.exists(cache_filename):
                print('[Text2Wave] Using cached sound file ({}) for {}'
                      .format(cache_filename, text.decode('utf-8')))
                shutil.copy(cache_filename, wavfile)
                sys.exit(0)
        ...
        with open(wavfilename, 'wb') as out:
            # Write the response to the output file.
            out.write(response.audio_content)
        if cache_enabled:
            text_cache_filename = os.path.splitext(cache_filename)[0] + '.txt'
            shutil.copy(txtfilename, text_cache_filename)
            shutil.copy(wavfilename, cache_filename)

    def determine_voice_name(self, voice_name):
    ...
    def checksum_md5(self, filename, blocksize=8192):
        """Calculate md5sum.
